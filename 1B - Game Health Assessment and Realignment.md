...
> *"Successful liveOps is only possible when the core gameplay has good retention by itself. It is not a support cost—it is almost a full development team with ongoing traffic acquisition."*
...
Games evolve. Over years, countless features, events, and pivots accumulate. The game players experience today may bear little resemblance to the original vision—sometimes for better, sometimes for worse. Understanding where you've drifted helps you decide: return to the original vision, embrace the evolution, or consciously chart a new direction.
...
```
                    Intentional Drift
                          │
           EVOLVED        │        PIVOTED
        (Planned growth)  │    (Strategic shift)
                          │
    ──────────────────────┼──────────────────────
                          │
          DECAYED         │         LOST
      (Quality erosion)   │    (Unplanned drift)
                          │
                   Unintentional Drift
```
...
1. **Dig up original documents** — GDD, pitch decks, launch marketing, early reviews
2. **Interview long-tenured team members** — What do they remember as the original intent?
3. **Read 100 recent player reviews** — How do players describe your game?
4. **Compare launch trailer to current gameplay** — Same game?
5. **Draft a "current vision" statement** — What is the game NOW?
...
Intuition is unreliable for games with years of history. Metrics reveal what's actually happening—not what you assume or remember. A comprehensive metrics audit surfaces trends, identifies problems, and establishes baselines for measuring improvement.
...
Compare cohort behavior over time—are new users better or worse than old ones?
...
| Do we understand WHY metrics are moving? | | Correlation ≠ causation |
...
3. **Identify the 3 most concerning trends** — Why are they happening?
4. **Identify the 3 healthiest metrics** — What's working?
...
Your players today may be very different from your launch audience. Demographics shift, early adopters churn, new segments discover the game. Understanding who actually plays—and who you're losing—informs feature priorities, content tone, and marketing targeting.
...
3. **Review 50 negative reviews** from last 6 months — who's complaining?
4. **Analyze your best players** — what makes them stay?
5. **Compare UA targeting to actual audience** — mismatch?
...
- Maintenance: High=1, Med=2, Low=3 (inverted—low cost is good)
...
```
                     High Usage
                         │
        CORE            │           OPTIMIZE
   (Protect & enhance)   │    (High value, high cost—
                         │     simplify or invest)
                         │
    ─────────────────────┼─────────────────────
                         │
        REVIEW           │           SUNSET
   (Low usage but may    │    (Low value, any cost—
    have strategic value)│     remove or archive)
                         │
                     Low Usage
...
| **REVIEW** | | Investigate—is low usage a discovery problem or disinterest? |
...
1. **List every feature** — be exhaustive, include "small" things
2. **Pull usage data** — what % of DAU touches each feature weekly?
3. **Estimate maintenance burden** — ask engineers and QA
4. **Place features in quadrants** — use data, not intuition
5. **Identify top 3 sunset candidates** — present to stakeholders
...
Technical debt compounds. Systems built quickly 5 years ago become bottlenecks today. Code that "works" may be fragile, slow, or incomprehensible to current team members. Mature games must actively manage tech debt or drown in it—every new feature becomes harder, every bug fix risks breaking something else.
...
```
                     High Impact
                         │
        CRITICAL         │        SCHEDULED
   (Pay down NOW—        │    (Plan for next
    blocking progress)   │     major release)
                         │
    ─────────────────────┼─────────────────────
                         │
        OPPORTUNISTIC    │        TOLERATE
   (Fix when touching    │    (Accept the cost,
    related code)        │     don't prioritize)
                         │
                     Low Impact
...
| **OPPORTUNISTIC** | | Boy Scout rule—leave better than found |
...
1. **Survey the engineering team** — What frustrates them? What's scary to touch?
2. **Review incident history** — What systems cause the most fires?
3. **Audit build and deploy times** — Where's the friction?
4. **Check dependency versions** — What's out of date?
...
Live games need continuous content to maintain engagement. But content production has real costs—team time, QA, localization, deployment. If your pipeline is inefficient, you either burn out the team, deliver less content, or ship lower quality. Understanding your true capacity prevents overcommitting.
...
3. **Survey the team** — What's frustrating about content production?
4. **Benchmark competitors** — How often do they update?
...
|--------|---------------|---------------|--------|
| Daily earn rate (median) | | | 🟢🟡🔴 |
| Daily earn rate (90th percentile) | | | 🟢🟡🔴 |
| Primary sinks | | | |
| Sink adequacy | | | 🟢🟡🔴 |
| Inflation rate (veteran stockpiles) | | <10%/year | 🟢🟡🔴 |
| New player friction | | | 🟢🟡🔴 |
...
| Tutorial → D1 active | | | |
| D1 → First store visit | | | |
| Store visit → First purchase | | | |
| First purchase → Second purchase | | | |
| Payer → Repeat payer (monthly) | | | |
...
3. **Analyze veteran currency stockpiles** — inflation indicator
...
|------------|--------|-------|---------------------------|--------------|
| | Active/Declining/Dead | ↑↓→ | | High/Med/Low |
| | | | | |
| | | | | |
| | | | | |
...
1. **Update competitor list** — Who's relevant now?
2. **Play 2 competitors** for 2+ hours each — What's changed?
3. **Read competitor reviews** — What do their players love/hate?
4. **Compare download/revenue trends** — Who's winning?
...
1. **Map knowledge to people** — Who knows what?
2. **Identify single points of failure** — What breaks if they leave?
3. **Audit documentation** — What exists, what's current, what's missing?
4. **Start a decision log** — Even recording decisions going forward helps
...
Adding is easier than removing. But every feature, system, and content type carries ongoing cost. Mature games must actively prune to stay healthy—freeing capacity, reducing complexity, and focusing on what matters. Identifying sunset candidates is the first step; executing sunsets requires planning.
...
```
   ...
   (Remove now,            │    (Remove now,
    minimal communication) │     simple announcement)
                           │
    ───────────────────────┼───────────────────────
                           │
        CAREFUL REMOVAL    │      STRATEGIC DECISION
   (Worth removing but     │    (High impact—must
    plan communication)    │     justify carefully)
                           │
                     High Player Impact
...
|----------|----------------------|----------------------|----------|
| Metrics decline 20%—cut costs or invest to fix? | | | |
| Competitor launches—copy features or differentiate? | | | |
| Major tech debt—delay features to fix? | | | |
| Team burnout risk—reduce output or add people? | | | |
| Revenue short of target—more aggressive monetization? | | | |
...
**Block 2B: Live Ops Planning & Optimization** — Improving event cadence, content velocity, and operational efficiency for mature games

OR

**Block 2A: Planning & Milestones** — If Block 1B revealed need for major new initiative
